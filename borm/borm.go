...
// Fields .
func (t *BormTable) Fields(fields ...string) *BormTable {

	w := &fieldsItem{Fields: fields}

	t.Args = append([]BormItem{w}, t.Args...)
	return t
}

// Join .
func (t *BormTable) Join(stmt string) *BormTable {

	w := &joinItem{Stmt: stmt}

	t.Args = append(t.Args, w)

	return t
}

// Where .
func (t *BormTable) Where(conds ...interface{}) *BormTable {

	if l := len(conds); l > 0 {
		if s, ok := conds[0].(string); ok {
			w := &whereItem{Conds: []interface{}{
				&ormCond{
					Op:   s,
					Args: conds[1:l],
				},
...
// GroupBy .
func (t *BormTable) GroupBy(fields ...string) *BormTable {
	w := &groupByItem{Fields: fields}
...
// Having .
func (t *BormTable) Having(conds ...interface{}) *BormTable {

	if l := len(conds); l > 0 {
		if s, ok := conds[0].(string); ok {
			w := &havingItem{Conds: []interface{}{
				&ormCond{
					Op:   s,
					Args: conds[1:l],
				},
...
// OrderBy .
func (t *BormTable) OrderBy(orders ...string) *BormTable {

	w := &orderByItem{Orders: orders}
...
// Limit .
func (t *BormTable) Limit(i ...interface{}) *BormTable {
	switch len(i) {
	case 1, 2:
		w := &limitItem{I: i}
...
func (t *BormTable) Get(query string, res interface{}) (int, error) {
	...
	if !isArray {
		// fire
		err := t.DB.QueryRowContext(t.ctx, query).Scan(item.Cols...)
		if err != nil {
...
func (t *BormTable) Select(res interface{}) (int, error) {
	...
	/*
		fmt.Println("select t.Cfg.Debug = ", t.Cfg.Debug)
		fmt.Println("select item.SQL = ", item.SQL)
	*/
...
// Select .
func (t *BormTable) MasterSelect(res interface{}) (int, error) {
...
func (t *BormTable) insert(prefix string, objs interface{}, args []BormItem) (int, error) {
	...
	if t.Cfg.Debug {
		query := fmt.Sprintf(strings.ReplaceAll(sb.String(), "?", "%v"), stmtArgs...)
		log.Println(query)
...
// Update .
func (t *BormTable) Update(obj interface{}) (int, error) {
...
// Delete .
func (t *BormTable) Delete() (int, error) {
...
func (t *BormTable) getStructFieldMap(s reflect2.StructType) map[string]reflect2.StructField {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		/*
			if f.Kind() == reflect.Struct && f.Anonymous == false {
				fmt.Println("getStructFieldMap reflect.Struct ")
			}
		*/
		ft := f.Tag().Get("borm")
...
func (w *fieldsItem) BuildString(sb *strings.Builder) {
...
func (w *onDuplicateKeyUpdateItem) BuildString(sb *strings.Builder) {
...
func (w *joinItem) BuildString(sb *strings.Builder) {
	sb.WriteString(" ")
...
func JoinWithFormatting(elements []interface{}, sep string) string {
	var strElements []string
	for _, elem := range elements {
		switch v := elem.(type) {
		case *int:
			strElements = append(strElements, fmt.Sprintf("%d", *v))
		case *float64:
			strElements = append(strElements, fmt.Sprintf("%.2f", *v))
		case *string:
			strElements = append(strElements, *v)
		default:
			strElements = append(strElements, fmt.Sprintf("%v", v))
		}
	}
...
func (w *whereItem) BuildString(sb *strings.Builder) {
	if len(w.Conds) <= 0 {
		return
	}
	...
	for i, c := range w.Conds {
		...
		if cond, ok := c.(*ormCond); ok {
			...
			fieldEscapeSQL(sb, cond.Field)
			//fmt.Println("cond.Op = ", cond.Op)
			if strings.HasPrefix(cond.Op, " between") {
				str := fmt.Sprintf(" between %d and %d ", cond.Args[0], cond.Args[1])
				sb.WriteString(str)
			} else if strings.HasPrefix(cond.Op, " in") {

				str := fmt.Sprintf(" in (%s) ", JoinWithFormatting(cond.Args, ","))
				sb.WriteString(str)

			} else {
				arr := strings.SplitN(cond.Op, "?", 3)
				if len(arr) > 1 {
					str := ""
					switch v := cond.Args[0].(type) {
					case int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64:
						str = fmt.Sprintf(" %s %d", arr[0], v)
					case string:
						str = fmt.Sprintf(" %s '%s'", arr[0], v)
					}
					sb.WriteString(str)
				} else {
...
func (w *groupByItem) BuildString(sb *strings.Builder) {
...
func (h *havingItem) BuildString(sb *strings.Builder) {
	if len(h.Conds) <= 0 {
		return
	}
...
func (o *orderByItem) BuildString(sb *strings.Builder) {
...
func (l *limitItem) BuildString(sb *strings.Builder) {

	var str string

	if len(l.I) > 1 {
		str = fmt.Sprintf(" limit %d  offset %d ", l.I[0], l.I[1])
	} else {
		str = fmt.Sprintf(" limit %d ", l.I[0])
	}
...
func numberToString(k reflect.Kind, src interface{}) string {
	...
	case reflect.Uint64:
		return fmt.Sprintf("%d", src.(uint64))
	case reflect.Float32:
		return fmt.Sprintf("%g", src.(float32))
